...
def write_image_summary(image_resolution, model, model_input, gt,
    ...
    # 设置更小的batch size来避免OOM
    step_size = 5000  # 减小batch size
    
    # 获取总坐标数量
    total_coords = model_input['coords'].shape[1]
    ...
    model.eval()
    with torch.no_grad():
        for i in range(0, total_coords, step_size):
            # 创建当前batch的输入
            model_input_tmp = model_input.copy()
            end_idx = min(i + step_size, total_coords)
            model_input_tmp['coords'] = model_input['coords'][:, i:end_idx, :].cuda()
            
            # 前向传播
            model_output_tmp = model(model_input_tmp)
            
            # 立即转移到CPU并存储
            pred_outputs.append(model_output_tmp['model_out'].cpu())
            
            # 清理GPU内存
            del model_input_tmp, model_output_tmp
            torch.cuda.empty_cache()
            
            # 如果内存压力很大，可以每几个batch强制清理一次
            if len(pred_outputs) % 5 == 0:
                torch.cuda.empty_cache()
    
    # 在CPU上拼接所有结果
    full_model_output = torch.cat(pred_outputs, dim=1)
    del pred_outputs  # 释放中间结果内存
    
    # 转换为图像格式
    pred_img = dataio.lin2img(full_model_output, image_resolution)
    gt_img = dataio.lin2img(gt['img'], image_resolution)
    
    # 处理预测图像用于显示
    pred_img_display = dataio.rescale_img((pred_img+1)/2, mode='clamp').permute(0,2,3,1).squeeze(0).detach().cpu().numpy()
    gt_img_display = dataio.rescale_img((gt_img+1) / 2, mode='clamp').permute(0, 2, 3, 1).squeeze(0).detach().cpu().numpy()
    
    # 写入tensorboard
    writer.add_image(prefix + 'pred_img', torch.from_numpy(pred_img_display).permute(2, 0, 1), global_step=total_steps)
    writer.add_image(prefix + 'gt_img', torch.from_numpy(gt_img_display).permute(2,0,1), global_step=total_steps)
    
    # 保存预测图片为png
    import torchvision
    cond_mkdir(os.path.join(os.path.dirname(writer.log_dir), 'results'))
    torchvision.utils.save_image(
        torch.from_numpy(pred_img_display).permute(2, 0, 1), 
        os.path.join(os.path.dirname(writer.log_dir), 'results', f'pred_img_{total_steps}.png')
    )
    ...
    # 清理内存
    del full_model_output, pred_img, gt_img, pred_img_display, gt_img_display
    torch.cuda.empty_cache()
...
def write_psnr(pred_img, gt_img, writer, iter, prefix):
    ...
    for i in range(batch_size):
        ...
        win_size = max(3, win_size)  # 确保至少是3

        # 使用新版本的 scikit-image API
        try:
            ssim = skimage.metrics.structural_similarity(p, trgt, multichannel=True, data_range=1, win_size=win_size)
            psnr = skimage.metrics.peak_signal_noise_ratio(p, trgt, data_range=1)
        except (AttributeError, ValueError):
            # 如果新版本API失败，尝试不指定win_size
            try:
                ssim = skimage.metrics.structural_similarity(p, trgt, multichannel=True, data_range=1)
                psnr = skimage.metrics.peak_signal_noise_ratio(p, trgt, data_range=1)
            except (AttributeError, ValueError):
                # 如果还是失败，跳过SSIM计算，只计算PSNR
                ssim = 0.0
                try:
                    psnr = skimage.metrics.peak_signal_noise_ratio(p, trgt, data_range=1)
                except (AttributeError, ValueError):
